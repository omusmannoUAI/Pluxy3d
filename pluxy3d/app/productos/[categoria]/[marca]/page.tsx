...
export default function ProductosPorMarcaPage({ params }: { params: { categoria: string; marca: string } }) {
	const { categoria, marca } = params
	...
	const filteredProducts = products.filter(
		(p) => p.category === categoria && p.brand.toLowerCase() === marca.toLowerCase()
	)

	return (
		<div className="container mx-auto px-4 py-8">
			<h1 className="text-3xl font-bold mb-8 capitalize">
				Productos / {categoria} / {marca}
			</h1>
					...
					<Card>
						...
						<CardContent className="space-y-6">
							...
							<div className="space-y-4">
								...
								<div className="space-y-2">
									<div className="flex items-center space-x-2">
										<Checkbox id="creality" checked={marca.toLowerCase() === "creality"} disabled />
										<Label htmlFor="creality">Creality</Label>
									</div>
									<div className="flex items-center space-x-2">
										<Checkbox id="hellbot" checked={marca.toLowerCase() === "hellbot"} disabled />
										<Label htmlFor="hellbot">Hellbot</Label>
									</div>
									<div className="flex items-center space-x-2">
										<Checkbox id="prusa" checked={marca.toLowerCase() === "prusa"} disabled />
					...
					<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
						{loading ? (
							...
							<p className="text-center py-8 text-muted-foreground w-full col-span-3">
								No hay productos para esta marca y categoría.
							</p>
						) : (
